...
void WdWddW_Spring3(
  ...
  const double C1 = u01[0] * C;
  const double C2 = u01[1] * C;
  const double C3 = u01[2] * C;
  Eigen::Vector3d ddC((C*C-C1*C1)/(C*C*C), (C*C-C2*C2)/(C*C*C), (C*C-C3*C3)/(C*C*C));
  ddw[0][0] = n - stiffness * C * ddC * u01.transpose();
  ddw[1][1] = n - stiffness * C * ddC * u01.transpose();
  ddw[0][1] = -n + stiffness * C * ddC* u01.transpose();
  ddw[1][0] = -n + stiffness * C * ddC* u01.transpose();
}
