...
void collide_particle_ball(
  ...
  const Eigen::Vector2f impulse = 2.f * particle_mass * ball_mass / (particle_mass + ball_mass) * (ball_pos - p.pos);
  p.velo += impulse.dot(plane_norm) / particle_mass * plane_norm;
  ball_velo += impulse.dot(plane_norm) / ball_mass * plane_norm;
}
