  ...
  while (!::glfwWindowShouldClose(window)) {
    if( time < 200.0 ) {
      for (int itr = 0; itr < 100; ++itr) { // sub-stepping
        ...
        rotation = dt * rotation * (Eigen::Matrix3f() << 0.f, Omega(2), -1 * Omega(1), -1.f * Omega(2), 0.f, Omega(0), Omega(1), -1.f * Omega(0), 0.f).finished() + rotation;
        Omega = -1.f * dt * inertia.inverse() * (Eigen::Matrix3f() << 0.f, Omega(2), -1.f * Omega(1), -1.f * Omega(2), 0.f, Omega(0), Omega(1), -1.f * Omega(0), 0.f).finished() * inertia * Omega + Omega;
